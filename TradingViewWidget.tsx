...
const TradingViewWidget: React.FC<TradingViewWidgetProps> = ({ lang }) => {
  ...
  return (
    <div className="relative group animate-soft-in">
      <div className="relative rounded-[2rem] overflow-hidden border border-white/10 bg-[#0b0e11] shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/10 bg-white/[0.02]">
           <div className="flex items-center gap-3">
             <div className="relative">
                <div className="w-2.5 h-2.5 rounded-full bg-[#10b981]"></div>
                <div className="absolute inset-0 w-2.5 h-2.5 rounded-full bg-[#10b981] animate-ping opacity-75"></div>
             </div>
             <span className="text-[11px] font-black uppercase tracking-[0.4em] text-white">{t.quantumFeed}</span>
           </div>
           <span className="text-[10px] font-black text-white uppercase tracking-widest opacity-60">{t.realTime}</span>
        </div>
